...
static int add_prio_qdisc(void)
{
	...
	/* prio_init() rejects a NULL opt, and prio_tune() reads TCA_OPTIONS as a
	 * bare struct tc_prio_qopt (not a nested attribute). */
	{
		struct { int bands; unsigned char priomap[16]; } qopt;

		memset(&qopt, 0, sizeof(qopt));
		qopt.bands = 3;
		nlb_put_attr(&b, TCA_OPTIONS, &qopt, sizeof(qopt));
	}
	return nl_send(nl, &b);
}
